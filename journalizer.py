...
def upsert_dir(state):
    if os.path.isdir(state['journal_dir']) and os.path.isfile(os.path.join(state['journal_dir'], str(state['journal_date'].day))):
        append_state = True
        return(append_state)
...
def write_journal(state, journal_entry, append_state):
    ...
    if append_state:
        try:
            with open(os.path.join(state['journal_dir'], str(state['journal_date'].day)), 'r') as file:
    ...
    final_write_state.append({
        'timestamp': str(state['journal_date'].time()),
        'content': journal_entry
    })
    
    try:
        with open(os.path.join(state['journal_dir'], str(state['journal_date'].day)), 'w') as file:
